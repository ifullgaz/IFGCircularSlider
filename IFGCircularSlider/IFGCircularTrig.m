...
@implementation IFGCircularTrig

+ (CGFloat)angleRelativeToNorthFromPoint:(CGPoint)fromPoint
                                 toPoint:(CGPoint)toPoint {
    return [self angleRelativeToNorthFromPoint:fromPoint toPoint:toPoint northAngle:0.0];
}

+ (CGFloat)angleRelativeToNorthFromPoint:(CGPoint)fromPoint
                                 toPoint:(CGPoint)toPoint
                             northAngle:(CGFloat)northAngle {
    ...
    double compassRadians = CartesianToCompass(cartesianRadians - ToRad(northAngle));
    while (compassRadians < 0) {
        compassRadians+= (2 * M_PI);
    }
    while (compassRadians > 2 * M_PI) {
        compassRadians-= (2 * M_PI);
    }
